...
def collect_image_files(
    ...
    """Return all supported image (and optionally video) files under *directory*.

    Args:
        directory:     Root directory to search.
        recursive:     When *True* descend into sub-directories.
        include_video: When *True* also include supported video files.
    ...
    if include_video:
        extensions = extensions | SUPPORTED_VIDEO_EXTENSIONS
    return sorted(
        p for p in glob if p.is_file() and p.suffix.lower() in extensions
    )
...
SUPPORTED_VIDEO_EXTENSIONS: frozenset[str] = frozenset(
    {".mp4", ".avi", ".mov", ".mkv", ".mp4v", ".m4v", ".wmv", ".flv"}
)
